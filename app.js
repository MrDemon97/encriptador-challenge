...
function cambiarTema() {
  ...
  /* seleccionamos los elementos que vamos a usar en el cambio de temas para el caso de las etiquetas by class
     sabesmo que estas devuelven una coleccion de elementos por tanto seleccionaremos el primer elemento*/
  ...
  //Primero debemos saber en que tema estamos para así poder redireccionar a determinado archivo css
  if (temaActual === 1) {
    //Si tema actual por defecto claro, pasamos a tema oscuro y cambiamos los iconos y la imagen del logo lupa
    //Definimos tema actual como 2
    temaActual = 2;
    //hacemos seleccionamos el archivo css del tema.
    etiquetaTema.href = "dark-theme.css";
    imagenLupa.src = "imgs/Muneco.png";
    iconoTema.src = "imgs/icons/samurai.png";

  } else if (temaActual === 2) {
    //si estamos en tema oscuro pasamos a tema samurai cat, por tanto definimos tema actual como tema 3
    temaActual = 3;
    ...
    //definimos tema actual como 1 y procedemos con los cambios
    temaActual = 1;
    imagenLupa.src = "imgs/Muneco.png";
     //hacemos seleccionamos el archivo css del tema.
...
function validarTexto() {
  ...
  if (textIngresado === "") {
    ...
    mostrarResultado(
      "error",
      "¡Recuerda! Sólo letras minúsculas y sin acentos."
    );
    return false;
